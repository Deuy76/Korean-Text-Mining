...
# Corpus 처리하기
mining.TXT.corpus <- corpus(X2011_News_data)

# Summary 보기
summary(mining.TXT.corpus$documents$texts)

mining.TXT.corpus <- mining.TXT.corpus[which(!grepl("세계일보",mining.TXT.corpus$documents$texts)),] # 특정 언론사 삭제
...
mining.TXT.token <- tokens_remove(mining.TXT.token, "[ㄱ-ㅎ]", 
                                  valuetype = 'regex', padding = TRUE)
# ㅠㅠ, ㅜㅜ 삭제
mining.TXT.token <- tokens_remove(mining.TXT.token, "(ㅜ|ㅠ)", 
                                  valuetype = 'regex', padding = TRUE)
...
topic<- LDA(convert(mining.TXT.token.dfm, to = "topicmodels"), k = 5)
